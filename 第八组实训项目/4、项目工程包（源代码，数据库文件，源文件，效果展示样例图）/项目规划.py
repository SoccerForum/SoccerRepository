...
整体项目分工：
	...
	目前完成的工作：
	搭建了完整的ssm框架，实现数据库数据的显示，可以实现前端页面的跳转
	还需要完成的：
	1.首先需要完善login页面，将登陆的数据同步到数据库
	2.登陆成功后转到首页，首页除了已经显示的球队信息外，还需要将数据库中的帖子信息呈现在中间的div中
	3.显示成功后，点击题目要显示帖子的内容，以及查询此帖子的visit的数据并显示
	4.与之相关的点赞，赞赏等功能也要同步到数据库中
	5.收藏后可以在个人界面查看
